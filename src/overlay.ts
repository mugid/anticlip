(() => {
  ...
  const featureLabels: Array<[keyof AntiClipConfig, string, string]> = [
  ...
  function normalizeConfig(value: unknown): AntiClipConfig {
    const partial = typeof value === "object" && value !== null
      ? value as Partial<AntiClipConfig>
      : {};
  ...
  function openOverlay() {
    ...
    panel = shadow.querySelector<HTMLElement>(".panel");
    const backdrop = shadow.querySelector<HTMLElement>("[data-backdrop]");
  ...
  chrome.runtime.onMessage.addListener((message) => {
    if (
      ...
      message === null ||
      (message as { type?: string }).type !== "TOGGLE_ANTICLIP_OVERLAY"
    ) {
      return;
    }
